...
for (taxon_level in c("species", "genus", "family", "order", "class")) {
    ...
    ## Instead order by mean genus abundance across all donors
    genus_hclust_order <- all_data_genus %>%
        group_by(.data[[taxon_level]]) %>%
        summarise(mean_abundance = mean(`Relative abundance`)) %>%
    ...
    all_data_genus <- all_data_genus %>%
        group_by(.data[[taxon_level]]) %>%
        filter(any(`Relative abundance` > abundance_cutoff))
