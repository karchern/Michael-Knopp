...
for (sc in c(
    ...
    for (comp in comparisons) {
        ...
        p <- ggplot() +
            geom_point(data = tmp %>% filter(!hit), aes(x = logFC, y = -log10(P.Value.limma)), color = "grey", alpha = 0.5, stroke = 0) +
            geom_point(data = tmp %>% filter(P.Value.limma < 0.05 & logFC > 1.5), aes(x = logFC, y = -log10(P.Value.limma)), color = "#5a88dd", alpha = 0.8, stroke = 0) +
            geom_point(data = tmp %>% filter(P.Value.limma < 0.05 & logFC < -1.5), aes(x = logFC, y = -log10(P.Value.limma)), color = "#b70b0b", alpha = 0.8, stroke = 0) +
