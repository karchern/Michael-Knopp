...
for (comp in comparisons) {
    ...
    p <- ggplot() +
        geom_point(data = tmp %>% filter(!hit), aes(x = logFC, y = -log10(P.Value)), color = "grey", alpha = 0.1) +
        geom_point(data = tmp %>% filter(hit), aes(x = logFC, y = -log10(P.Value)), color = "red", alpha = 0.5) +
        geom_text_repel(
            data = tmp %>% filter(hit),
