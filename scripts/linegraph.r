...
filtered_data <- original_data %>% 
  ...
  mutate(SampleID = row_number(),
    Category = case_when(
      GFP == 'ompK35' & RFP == 'wt' ~ 'wt(RFP) vs ompK35(GFP)', #if this combo, give this in catogry col
      GFP == 'wt' & RFP == 'ompK35' ~ 'wt(GFP) vs ompK35(RFP)', #if this combo, give this in catogry col
...
#pivot data
long_data <- pivot_longer(filtered_data, cols = starts_with('ratio'), #columns with names that start with Kpn_abundance
  names_to = 'generation',  
  values_to = 'ratio') %>%
  mutate(generation = str_remove(generation, 'ratio_mut_over_wt_'))
